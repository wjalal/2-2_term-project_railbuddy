...
<script>
    ...
    onMount (event => {
		...
		axios.post(`${server}/api/getComplaintsAdmin`, {
		}).then(res => {
            ...
            {#if !stop.res_text}
                <div class='d-flex flex-column px-md-5 w-100 my-md-0 my-4'>
                        <div class='comp-detail mx-auto'>
                            {#if stop.associated_station}
                                <p class='h3'>#{stop.complaint_id}</p>
                                <p class='h5'>{stop.req_time}</p>
                                <p>{stop.name} ({stop.user_mobile})</p>
                            {/if}
                            {#if stop.associated_station}
                                <b>Associated Station: </b>
                                <p>{stop.associated_station}</p>
                            {/if}
                            {#if stop.associated_train}
                                <b>Associated Train: </b>
                                <p>{stop.associated_train}</p>
                            {/if}
                            <hr>
